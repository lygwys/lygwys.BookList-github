...
import {CloudBookListServiceProxy, PagedResultDtoOfCloudBookListListDto, CloudBookListListDto } from '@shared/service-proxies/service-proxies';
...
export class  CloudBookListComponent extends PagedListingComponentBase<CloudBookListListDto>
	implements OnInit {
	...
	*/
	protected fetchDataList(request: PagedRequestDto,pageNumber: number,finishedCallback: Function): void {
		this._cloudBookListService.getPaged(
			this.filterText,
			request.sorting,
	...
	*/
	createOrEdit(id?: number): void {
		this.modalHelper.static(CreateOrEditCloudBookListComponent, { id: id })
		.subscribe(result => {
			if (result) {
				this.refresh();
			}
	...
	*/
	delete(entity: CloudBookListListDto): void {
		this._cloudBookListService.delete(entity.id)
		.subscribe(() => {
		/**
